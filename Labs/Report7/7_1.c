...
int main(void)
{
  ...
  for (;;)
  {
    ...
    // Converting the ids to ascii
    char man_ascii[3];
    man_ascii[0] = (char)((manufacturerID >> 8) & 0xFF); // High byte
    man_ascii[1] = (char)(manufacturerID & 0xFF); // Low byte
    man_ascii[2] = '\0';

    // transmitting the data
    uart_write_string("Manufacturer ID: ");
    uart_write_string(man_ascii);
    uart_write_char('\n');

    // Converting the device id.
    char dev_ascii[5];
    uint16_to_4hex(deviceID, dev_ascii);
    uart_write_string("Device ID: ");
    uart_write_string(dev_ascii);
    uart_write_char('\n');
