...
__interrupt void signal_response()
{
    ...
    TA0CCTL0 &= ~CCIFG; // use TAIFG by default when timer is in continuous
}
...
int main(void)
{
  ...
  TA0CCTL0 |= CCIE; // enabling the channel 0
  TA0CCTL0 &= ~CCIFG; // clearing flag for channel 0

	// Lowest power mode that keeps ACLK enabled
	_low_power_mode_3();
